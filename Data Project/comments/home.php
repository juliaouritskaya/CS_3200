<?php
session_start();

$userid= $_GET['id'];
?>
<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Comments</title>
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.4.1/css/all.css"
        integrity="sha384-5sAR7xN1Nv6T6+dT2mhtzEpVJvfS3NScPQTrOxhwjIuvcA67KV2R5Jz6kr4abQsz" crossorigin="anonymous">

    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.4/jquery.min.js"></script>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM" crossorigin="anonymous">

        <script src="myjs.js"></script>
    <style>

        

    .fa-thumbs-up:hover,.fa-edit:hover {
        color: teal;
    }
    .fa-trash-alt:hover{
        color:red;
    }


    </style>


</head>

<body>
    <nav class="navbar navbar-expand-lg bg-body-tertiary">
        <div class="container-fluid">
            <a class="navbar-brand" href="#">Home</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav"
                aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" href="home.php?id=<?php echo $userid;
?>"> Posts</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="addpost.php?id=<?php echo $userid;
?>">Add Posts</a>
                    </li>
                    <li class="nav-item">
                    <a class="nav-link"  href="myposts.php?id=<?php echo $userid;
?>">My Posts</a>
                    </li>

                    <li class="nav-item">
                    <a class="nav-link"  href="news.php?id=<?php echo $userid;
?>">News</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link" href="login.php">Logout</a>
                    </li>

                </ul>
            </div>
        </div>
    </nav>


    <div class="row d-flex justify-content-center">
        <div class="col-md-8 col-lg-6">
            <div class="card shadow-0 border" style="background-color: #f0f2f5;">
                <div class="card-body p-4">
                   
                    <?php
                    include "conn.php";

                    $sql = "SELECT posts.id,posts.description,users.username FROM socialapp.posts inner join users on users.id=posts.userid;";
   
                   
                   
   
                    $result = mysqli_query($conn,$sql);
    $row = mysqli_fetch_array($result,MYSQLI_ASSOC);
    
    
    $count = mysqli_num_rows($result);
    
    // If result matched $myusername and $mypassword, table row must be 1 row
      
    $result = $conn->query($sql);

if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {



    echo     ' <div class="card mb-4">
    <div class="card-body">
        <p>'.$row["description"].'</p>

        <div class="d-flex justify-content-between">
            <div class="d-flex flex-row align-items-center">
                <img src="data:image/jpeg;base64,"
                    alt="avatar" width="25" height="25" />
                <p class="small mb-0 ms-2">'.$row["username"].'</p>

            </div>
            <div class="d-flex flex-row align-items-center">

                <i class="fas fa-thumbs-up" onclick=likepost('.$row['id'].','.$userid.')></i> &nbsp &nbsp &nbsp &nbsp
                <br>
                <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="showmodal('.$row['id'].')"
                >Comment</button>&nbsp &nbsp &nbsp &nbsp
 <input type="text" name="type"  value="'.$row['id'].'" id="postid'.$row['id'].'" hidden/>
 <input type="text" name="type"  value="'.$userid.'"  id="userid'.$row['id'].'" hidden/>';

 $countlikessql="SELECT count(*) as count FROM `likes` where post_id=".$row['id']."";
 $resultcount = $conn->query($countlikessql);
 $count="0";

 if ($resultcount->num_rows > 0) {
   // output data of each row
   while($rowcount = $resultcount->fetch_assoc()) {
  echo'  <p class="small text-muted mb-0">'.$rowcount["count"].'</p>';
   }
}
 



 echo '
               
            </div>


        </div>
        <br>

        
       


';
        $pid= $row["id"];
        $sql2 = "SELECT comments.id,comments.description ,users.username,comments.created_at,comments.userid FROM `comments` inner join users on users.id=comments.userid WHERE comments.post_id=".$pid."";
      
        
       
        
        // If result matched $myusername and $mypassword, table row must be 1 row
          
        $result2 = $conn->query($sql2);
    
    if ($result2->num_rows > 0) {
      // output data of each row
      while($row2 = $result2->fetch_assoc()) {

        if(!$row2["userid"]==$userid){
        echo'

        <div class="d-flex flex-row align-items-center">
            <p style="font-size:12px;">'.$row2['description'].' &nbsp &nbsp &nbsp
                &nbsp <b style="font-size:10px;">'.$row2['username'].' &nbsp &nbsp &nbsp &nbsp '.$row2['created_at'].'</b></p></div>
       
      ';}
      else{
        echo'

        <div class="d-flex flex-row align-items-center">
            <p style="font-size:12px;">'.$row2['description'].' &nbsp &nbsp &nbsp
                &nbsp <b style="font-size:10px;">'.$row2['username'].' &nbsp &nbsp &nbsp &nbsp '.$row2['created_at'].'</b></p>

                &nbsp &nbsp &nbsp &nbsp <p style="font-size:12px;"><i class="fas fa-trash-alt" onclick="deletecomment('.$row2["id"].')" ></i></p>
                
                
                </div>
       
      ';
      }}} echo '
    








         
        
    </div>
</div>';

  }
} 
    ?>





                </div>
            </div>
        </div>
    </div>



    <!-- Modal -->
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Comment</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">

                    <div class="input-group mb-3">
                        <span class="input-group-text" id="inputGroup-sizing-default">Comment</span>
                        <input type="text" class="form-control" aria-label="Sizing example input" id="commentcontent"
                            aria-describedby="inputGroup-sizing-default">
                    </div>
                </div>
                <div class="modal-footer" id="modalfooter">
                   
                </div>
            </div>
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-geWF76RCwLtnZ8qwWowPQNguL3RmwHVBC9FhGdlKrxdiJJigb/j/68SIy3Te4Bkz" crossorigin="anonymous">
    </script>
</body>

</html>